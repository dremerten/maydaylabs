...
if kubectl get statefulset database -n "$NAMESPACE" &>/dev/null; then
    READY=$(kubectl get statefulset database -n "$NAMESPACE" -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
    DESIRED=$(kubectl get statefulset database -n "$NAMESPACE" -o jsonpath='{.spec.replicas}' 2>/dev/null)
    ...
    if [ "$READY" = "$DESIRED" ]; then
        ...
        POD_NAMES=$(kubectl get pods -n "$NAMESPACE" -l app=database -o jsonpath='{.items[*].metadata.name}' 2>/dev/null)
