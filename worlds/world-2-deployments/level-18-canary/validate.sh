...
# Get replica counts
STABLE=$(kubectl get deployment app-stable -n "$NAMESPACE" -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
CANARY=$(kubectl get deployment app-canary -n "$NAMESPACE" -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
