...
ENDPOINTS=$(kubectl get endpoints backend-service -n "$NAMESPACE" -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null)
...
RESPONSE=$(kubectl exec test-client -n "$NAMESPACE" -- curl -s -o /dev/null -w "%{http_code}" http://backend-service 2>/dev/null)
